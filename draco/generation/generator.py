...
def main():
    ...
    model = Model(
        distributions, definitions['topLevelProps'], definitions['encodingProps'])
        ...
        if (interaction['include']):
            ...
            for j in tqdm(range(NUM_GROUPS)):
                ...
                for d in tqdm(range(1, 4)):
                    specs = generate_interaction(
                        model, dummy_data, interaction, d)
    ...
    print('{0} specs with {1} pairs for {2} interactions'.format(
        count, pairs, len(interactions)))
