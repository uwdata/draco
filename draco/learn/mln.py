...
def load_data(input_dir, format="compassql"):
    ...
    result = {}
    for fname in files:
        if (("rank-by-feature_histogram.json" in fname) or ("1d-T.json" in fname) 
            or ("1d-N.json" in fname) or ("1d-N-mark.json" in fname) or ("1d-O.json" in fname) 
            or ("2d-QxT.json" in fname) or ("3d-OxQxQ.json" in fname) or ("3d-NxQxQ.json" in fname)
            or ("bin-maxbins.json" in fname)):
            continue
...
def discriminative_learning(train_data, initial_weights, learning_rate=0.01, max_iter=10):
    """ discriminative learning for mln from partial and full specs """

    weights = copy.copy(initial_weights) # a shallow copy
